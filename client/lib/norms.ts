...
function isCardio(name: string) {
  const n = name.toLowerCase();
  return (
    n.includes('step') || n.includes('cardio') || n.includes('treadmill') || n.includes('mcaft') || n.includes('kasch')
  );
}
...
export function inferNormsForTest(testName: string): NormInfo {
  if (!testName) return { unit: '', left: null, right: null, category: 'other' };
  ...
  if (name.includes('lift') || name.includes('carry') || name.includes('push') || name.includes('pull') || name.includes('strength') || name.includes('force')) {
    return { unit: 'lb', left: 85.0, right: 90.0, category: 'strength' };
  }
